...
const singleValueNewMov = (tableTMov, fecha, concepto, nroCbte, valor, smConcepto, credito, idUsu, descripcion) => {
    return ` ('${fecha}', '${concepto}', '${nroCbte}', '${valor}', (SELECT orden as Ord FROM ${tableTMov} WHERE desc_extracto LIKE '${smConcepto}' order by id desc LIMIT 1), ${credito}, '0', '0', ${idUsu}, '0', '${descripcion}') `
}
...
const movimientosBco = (desde, hasta, detalle, filtro, pagAct, tabla = "extractos_bco_cba") => {
    if (detalle) {
        const desdePag = ((parseInt(pagAct) - 1) * 10)
        if (filtro) {
            return ` SELECT * FROM ${tabla} WHERE fecha >= '${desde}' AND fecha <= '${hasta}' AND saldo_ini = 0 AND (concepto LIKE '%${filtro}%' OR concepto LIKE '%${filtro}%' OR nro_cbte LIKE '%${filtro}%' OR monto = '${parseFloat(filtro)}') ORDER BY fecha, id_tipo, nro_cbte, concepto ASC LIMIT ${desdePag}, 10 `
        } else {
            return ` SELECT * FROM ${tabla} WHERE fecha >= '${desde}' AND fecha <= '${hasta}' AND saldo_ini = 0 ORDER BY fecha, id_tipo, nro_cbte, concepto ASC LIMIT ${desdePag}, 10 `
        }
    } else {
        return ` SELECT * FROM ${tabla} WHERE fecha >= '${desde}' AND fecha <= '${hasta}' AND saldo_ini = 0 ORDER BY fecha, id_tipo, nro_cbte, concepto ASC `
    }
}
