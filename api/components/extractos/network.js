...
const fecha = (req, res, next) => {
    Controller.fecha(req.body.fechaN, req.body.fechaV)
        .then(() => {
...
const get3 = (req, res, next) => {
    Controller.getMovimientos3(req.params.page, req.query.desde, req.query.hasta, req.query.filtro)
        .then((list) => {
            response.success(req, res, 200, list)
        })
        .catch(next)
...
const aliviarBD = async (req, res, next) => {
    Controller.aliviarBD(req.query.hasta)
        .then((list) => {
            response.success(req, res, 200, list)
        })
        .catch(next)
...
const ajustarSaldo = async (req, res, next) => {
    Controller.ajustarSaldo(req.query.saldo)
        .then((list) => {
            response.success(req, res, 200, list)
        })
        .catch(next)
...
const nuevoImpGsto = async (req, res, next) => {
    console.log('req.body :>> ', req.body.importe);
    Controller.nuevoImpGsto(req.body.tipo, req.body.importe, req.body.fecha)
        .then((list) => {
            response.success(req, res, 200, list)
        })
        .catch(next)
...
router.post("/gastosImpuesto", secure(3), nuevoImpGsto)
router.put("/fecha", secure(), fecha)
...
router.get("/aliviarBD", secure(3), aliviarBD)
router.get("/tiposMov", secure(3), listTiposMov)
router.get("/ajustarSaldo", secure(3), ajustarSaldo)
...
router.get("/busquedaBack/:page", secure(3), get3)
